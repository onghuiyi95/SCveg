...
<head>
  ...
  <title>SunnyChoice PO System</title>
...
<body class="bg-gray-100">
  ...
  <script type="text/babel">
    ...
    // === SVG 圖標元件替換 (移除對 lucide-react 的依賴) ===
    ...
    const IconSettings2 = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M20 7h-9"/><path d="M14 17H5"/><circle cx="17" cy="17" r="3"/><circle cx="7" cy="7" r="3"/></svg>;
          ...
          <header className="bg-slate-800 text-white shadow-md sticky top-0 z-30 hide-on-print">
            <div className="max-w-[1600px] mx-auto px-4 py-3 flex justify-between items-center">
              <h1 className="text-lg md:text-xl font-bold flex items-center gap-2">
                <IconFileText className="w-5 h-5 md:w-6 md:h-6 text-emerald-400" />
            ...
            <div className="w-full lg:w-[400px] xl:w-[450px] flex flex-col gap-4 hide-on-print lg:sticky lg:top-20 z-10 shrink-0">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                <h2 className="font-bold text-gray-800 flex items-center gap-2 mb-4 border-b pb-2">
                  <IconSettings2 className="w-5 h-5 text-slate-500" /> 单据基础设置
                </h2>
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1 flex items-center gap-1"><IconReceipt className="w-3 h-3"/> Billing Address (账单地址)</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1 flex items-center gap-1"><IconMapPin className="w-3 h-3"/> Delivery Address (送货地址)</label>
                      ...
                      <div key={product.id} className={`flex items-center justify-between p-2.5 rounded border transition-colors ${qty > 0 ? 'border-emerald-400 bg-emerald-50/50 shadow-sm' : 'border-transparent hover:bg-gray-100'}`}>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center border border-gray-300 rounded-md bg-white shadow-sm">
                            <button onClick={() => decrement(product.id)} className="px-2.5 py-1.5 text-gray-600 hover:bg-gray-100 rounded-l-md"><IconMinus className="w-3.5 h-3.5" /></button>
                            ...
                            <button onClick={() => increment(product.id)} className="px-2.5 py-1.5 text-gray-600 hover:bg-gray-100 rounded-r-md"><IconPlus className="w-3.5 h-3.5" /></button>
                          </div>
                        </div>
                      </div>
                    );
    ...
    root.render(<PurchaseOrderApp />);
  </script>

  <!-- 全局 CSS 樣式 -->
  <style>
    /* A4 紙張尺寸設定 (210x297mm) */
    .a4-paper { width: 210mm; min-height: 297mm; margin: 0 auto; }
    /* 隱藏滾動條 */
    ...
    @media print {
      ...
      .print-area-container { width: 100% !important; margin: 0 !important; padding: 0 !important; overflow: visible !important; justify-content: flex-start !important; }
      .a4-paper { width: 100% !important; box-shadow: none !important; margin: 0 !important; border: none !important; min-height: auto !important; }
